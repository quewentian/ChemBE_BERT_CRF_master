...
from myHTTP import BertHTTPProxy
# from bert_base.server.http import BertHTTPProxy
### File "/home/lzhpc/BERT-BiLSTM-CRF-NER/bert_base/server/http.py", line 5, in <module>
#     from .helper import set_logger
# SystemError: Parent module '' not loaded, cannot perform relative import###报错

# from bert_base.server.zmq_decor import multi_socket
from my_zmq_decor import multi_socket
sys.path.append('..')
...
class BertServer(threading.Thread):
    def __init__(self, args):
        ...
        if args.mode == 'BERT':
            ...
            with Pool(processes=1) as pool:
                ...
                from graph_soft import optimize_bert_graph
            ...
            with Pool(processes=1) as pool:
                # optimize the graph, must be done in another process
                from graph_soft import optimize_ner_model
                ######model_dir: trained model###
                num_labels, label2id, id2label = init_predict_var(self.args.model_dir)
                print('init the graph first, id2labels:   ',id2label)
                self.num_labels = num_labels + 1
                self.id2label = id2label
                ###path of pb file：graph_path
            ...
            with Pool(processes=1) as pool:
                # optimize the graph, must be done in another process
                from graph_soft import optimize_class_model
                num_labels, label2id, id2label = init_predict_var(self.args.model_dir)
    ...
    def _run(self, _, frontend, sink, *backend_socks):
        ...
        # BertSink：Used to receive the output of the upper BertWork，then send to client
        ...
        ###这里的编码不确定
        addr_sink = sink.recv().decode('UTF-8')
        # print('bert_server-run()-recv   111')

        # start the backend processes
        # BertWorker:Start multiple processes, load pb model
        ...
        while True:
            try:
                ...
                print(req_id,'   bert_server-run()-recv   222')
            except ValueError:
...
class BertSink(Process):
    ...
    def _run(self, receiver, frontend, sender):
        ...
        frontend.send(receiver_addr.encode('UTF-8'))
        ...
        while not self.exit_flag.is_set():
            ...
            if socks.get(receiver) == zmq.POLLIN:
                ...
                print(msg, '   BertSink-_run()-receiver-recv   333')
                job_id = msg[0]
                ...
                print('pending_checksum:     ',pending_checksum)
                print('job_checksum:     ',job_checksum)
                for k,v in pending_result.items():
                    print('pending_result k:   ',k)
                    print('pending_result v:   ', v)
            ...
            if socks.get(frontend) == zmq.POLLIN:
                ...
                print(client_addr, '   BertSink-_run()-frontend-recv   444')
...
class BertWorker(Process):
    ...
    def _run(self, sink, *receivers):
        ...
        ##I:WORKER-1:[mys:_ru:513]:use device gpu: 1, load graph from /home/lzhpc/0BERT-NER-master/relationOutPre/ner_model.pb
        tf = import_tf(self.device_id, self.verbose, use_fp16=self.use_fp16)
        estimator = self.get_estimator(tf)####得到不同Bert模型（NER/Classification）的Estimator模型
        for sock, addr in zip(receivers, self.worker_address):
            sock.connect(addr)
        #################################新加#############################
        # sink_embed.connect(self.sink_address)
        # sink_token.connect(self.sink_address)########接受bertWorker的产出，发送给client
        #################################新加#############################
        print(self.sink_address,'   sink_address%%')
        sink.connect(self.sink_address)
        print('sink has connect address')
        # print(self.input_fn_builder(receivers, tf, sink),'   print error here%%%???')
        # print(estimator.predict(input_fn=self.input_fn_builder(receivers, tf, sink_token), yield_single_examples=False)),'  test%%'
        for r in estimator.predict(input_fn=self.input_fn_builder(receivers, tf), yield_single_examples=False):
            if self.mode == 'BERT':
                send_ndarray(sink, r['client_id'], r['encodes'], ServerCommand.data_embed)
                logger.info('job done\tsize: %s\tclient: %s' % (r['encodes'].shape, r['client_id']))
            elif self.mode == 'NER':
                ######将结果id转成真是的标签
                print('in BertWorker r=   ',r['encodes'])
                print('r   ',r)
                # pred_label_result=['<O>','<B-PER>','<O>','<B-PER>']
                # pred_ids_result=[0,1,0,1]
                # pred_label_result, pred_ids_result = ner_result_to_json(r['encodes'], self.id2label)
                ###ner_result_to_json这里先注释掉，修改后面的函数，直接返回待遇###号的预测
                pred_label_result, pred_ids_result = ner_result_to_json1(r['encodes'], self.id2label)
                print(pred_label_result,'   pred_label_result&&')
                print(pred_ids_result,'   pred_ids_result')
                print('self.tokens before send back:    ',self.tokens)
                # rst = send_ndarray(sink, r['client_id'], pred_label_result, ServerCommand.data_embed)
                ###返回predict_dict={tokens，预测标签}的词典
                predict_dict={'tokens':self.tokens,'pred_label':pred_label_result}
                print('predict_dict!!!:     ',predict_dict)
                rst = send_ndarray(sink, r['client_id'], predict_dict, ServerCommand.data_embed)
                print('rst:   ', rst)
                logger.info('job done\tsize: %s\tclient: %s' % (r['encodes'].shape, r['client_id']))
            elif self.mode == 'CLASS':
    ...
    def input_fn_builder(self, socks, tf):
        import sys
        sys.path.append('..')
        from my_extract_features import convert_lst_to_features
        # from bert_base.bert.tokenization import FullTokenizer
        from myTokenization import FullTokenizer
        ...
        def gen():
            ...
            while not self.exit_flag.is_set():
                ...
                for sock_idx, sock in enumerate(socks):
                    if sock in events:
                        #接收来自客户端的消息recv_multipart()
                        client_id, raw_msg = sock.recv_multipart()
                        msg = jsonapi.loads(raw_msg)
                        print(msg,'   msg!!!')
                        # ['second dissociation constant have been measured spectrophotometrically',
                        ...
                        max_position_embeddings1 = 512
                        ###convert_lst_to_features:将server端接收到的msg转化成可以输入的estimator模型计算的向量
                        ###返回的张量由tokens，input_ids，input_mask，input_type_ids
                        tmp_f = list(convert_lst_to_features(msg, self.max_seq_len, max_position_embeddings1,tokenizer, logger,is_tokenized, self.mask_cls_sep,False))

                        print(client_id,'   client_id!!!')
                        print([f.tokens for f in tmp_f], '   tokens@@')
                        self.tokens=[f.tokens for f in tmp_f]
                        print(type(self.tokens))
                        print([f.input_ids for f in tmp_f],'   input_ids@@')
                        print([f.input_mask for f in tmp_f],'   input_mask$$')
                        try:
                            print( [f.input_type_ids for f in tmp_f],'   input_type_ids')
                        except:
                            print('no input_type_ids')
                        print(type(f.input_ids for f in tmp_f))        ###<class 'generator'>我一直以为他就是一个普通的list，要打印出来看看！
                        # genDict={
                        #     'client_id': client_id,
                    ...
                    ###generator与前面的yield是对应的，yield产生的数据格式，就是generator格式，所以，换成return还不行
        ...
        def input_fn():
            ...
            #         'input_type_ids': (None, self.max_seq_len)}).prefetch(self.prefetch_size))
            # print(type((tf.data.Dataset.from_generator(
            #     gen,
            #     output_types={'input_ids': tf.int32,
            #                   'input_mask': tf.int32,
            ...
            return (tf.data.Dataset.from_generator(
                ...
                output_shapes={
                    ...
                    'input_ids': (None, self.max_seq_len),
                    'input_mask': (None, self.max_seq_len),
                    'segment_ids': (None, self.max_seq_len) })) # .shard(num_shards=4, index=4)
        return input_fn
...
def init_predict_var(path):
    """
    初始化NER所需要的一些辅助数据：label数，id与label互相映射的词典
    :param path:
    :return:
    """
...
def ner_result_to_json1(predict_ids, id2label):
    """
    ...
    pred_label_result, pred_ids_result =\
                convert_id_to_label2(predict_ids, id2label, len(predict_ids))
    print(predict_ids,'       predict_ids')
    print(id2label,'    id2label')
    print(len(predict_ids),'    len predict_ids')
    return pred_label_result, pred_ids_result

def convert_id_to_label2(pred_ids_result, idx2label, batch_size):
    """
    将id形式的结果转化为真实序列结果
    ...
    for row in range(batch_size):
        curr_seq = []###句子的游标
        curr_idx = []###每个句子中id 游标
        ids = pred_ids_result[row]###每个句子分别取出来，这里的ids就是一个句子
        for idx, id in enumerate(ids):
            if id == 0:###0就是没有词，[PAD]的
                break
            curr_label = idx2label[id]###从对应的map中取出对应的label来
            ##############直接看tokens，这里先不去掉##################
            # if curr_label in ['[CLS]', '[SEP]','X']:
