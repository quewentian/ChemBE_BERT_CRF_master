...
def convert_lst_to_features(lst_str, max_seq_length, max_position_embeddings,
    ...
    print(examples,'   examples???')#<generator object read_examples at 0x7f8fe16da620>    examples???
    print( lambda x: tokenizer.mark_unk_tokens(x))
    ...
    for (tokens_a, tokens_b) in all_tokens:
        ...
        logger.debug('tokens: %s' % ' '.join([myTokenization.printable_text(x) for x in tokens]))
...
def read_examples(lst_strs):
    ...
    print('read_examples    ,here!')
    unique_id = 0
    for ss in lst_strs:
        print('ss:   ',ss)
        # ss: ['second', 'dissociation', 'constant', 'have', 'been', 'measured', 'spectrophotometrically', 'of', 'phenyl',
        #      'benoates']
        print(type(ss))
        # <class 'list'>
        line = myTokenization.convert_to_unicode(ss)
        print(line,'    line!!!')
        if not line:
            continue
        line = line.strip()
...
def read_tokenized_examples(lst_strs):
    print('read_tokenized_examples     here!')
    unique_id = 0
    lst_strs = [[myTokenization.convert_to_unicode(w) for w in s] for s in lst_strs]
    for ss in lst_strs:
        text_a = ss
        text_b = None
        try:
